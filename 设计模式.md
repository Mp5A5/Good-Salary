...
* ObjectStructure:对象结构。能够枚举它的元素，可以提供一个高层的接口来允许访问者访问它的元素。(可以按照业务逻辑来设计，可有可无)
 ...
 * 员工们就是参与考核的人。他们给供访问者查看内部属性的方法
 */
abstract class Staff(val name: String, var kpi: Int = Random().nextInt(10)) {
    // 接受Visitor访问
...
class Engineer(name: String) : Staff(name) {
    ...
    fun getCodeLines(): Int = Random().nextInt(10 * 10000)
}
...
class Manager(name: String) : Staff(name) {
    ...
    fun getProducts(): Int = Random().nextInt(10)
}
```
...
class CTO : Visitor {
    override fun visit(engineer: Engineer?) {
        println("工程师：" + engineer?.name + ",代码量:" + engineer?.getCodeLines());
    }

    override fun visit(manager: Manager?) {
        println("经理：" + manager?.name + ",产品量:" + manager?.getProducts());
    }
}
```
...
class BusinessReport() {
    ...
    init {
        staffs.add(Manager("李经理"))
        staffs.add(Manager("陈经理"))
        staffs.add(Engineer("java-李四"))
        staffs.add(Engineer("python-张安"))
        staffs.add(Engineer("C++-赵伟"))
    }
...
经理：李经理,产品量:0
经理：陈经理,产品量:4
工程师：java-李四,代码量:8388
工程师：python-张安,代码量:760
工程师：C++-赵伟,代码量:82576
```
...
public static void main(String[] args) {
    ...
    originator.setState("eat food  now ！");
    ...
    originator.setState("play game now !");
    printState(originator);
...
抽象表达式:抽象的解释操作父类，并定义一个抽象的解释方法
终结符表达式:实现文法中与元素相关联的解释操作，比如 A + B 这里A,B是终结符
非终结符表达式:实现文法中与非终结符有关的解释操作，非终结符表达式一般是文法中的运算符或者其他关键字
环境类:包含解释器之外的全局信息
